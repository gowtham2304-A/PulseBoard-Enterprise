...
export function KanbanBoard({ tasks, onTaskClick, onManualMove, viewMode, onSimulateInactivity, onSetDeadline }) {
  if (viewMode === 'swimlanes') {
    return (
      ...
      export function KanbanBoard({ tasks = [], columns = COLUMNS, onTaskClick, onManualMove, viewMode = 'columns', onSimulateInactivity, onSetDeadline }) {
        if (viewMode === 'swimlanes') {
          return (
        ...
        return (
          <div className="flex gap-3 px-3 py-4 overflow-x-auto kanban-scroll">
            {columns.map((col) => (
              <div key={col.id} className="w-[200px] sm:w-[230px] md:w-[260px] lg:w-[300px] xl:w-[320px] kanban-column rounded-2xl p-3 flex-shrink-0">
